/**
 * Thin client for the n8n internal REST API, used by the local-run scripts.
 *
 ...
 */
const BASE = process.env.N8N_BASE || `http://127.0.0.1:${process.env.N8N_HOST_PORT || 5679}`;
...
export async function login({ email, password }) {
  const res = await fetch(`${BASE}/rest/login`, {
    method: 'POST',
    headers: { 'content-type': 'application/json', 'browser-id': 'local-run' },
